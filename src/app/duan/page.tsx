    ...
    <div className="min-h-screen bg-[#0b1326] text-white font-sans pb-32">
      <nav className="flex items-center justify-between p-4 sm:p-8 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-[10px] text-gray-400 tracking-widest mt-1 uppercase">Cảnh Quan & Công Trình</span>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 mb-12">
        <div className="relative h-64 md:h-80 rounded-3xl overflow-hidden">
          ...
          <div className="absolute bottom-8 left-8 right-8">
            ...
            <p className="text-gray-300 text-sm max-w-xl">Cung ứng hoa giấy quy mô lớn cho Resort, Khu đô thị và Dự án hạ tầng xanh trên toàn quốc.</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PROJECTS.map((p) => (
            <div key={p.name} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-indigo-500/50 transition-all group">
              ...
              <div className="p-4">
                <h4 className="font-black text-lg mb-2 uppercase">{p.name}</h4>
                <p className="text-xs text-gray-400 leading-relaxed mb-4">{p.desc}</p>
                <div className="flex flex-wrap gap-2">
                  {p.tags.map(t => (
                    <span key={t} className="text-[9px] bg-indigo-500/20 text-indigo-300 px-2 py-1 rounded-full font-black uppercase tracking-wider">{t}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
          <table className="w-full text-left">
            ...
            <tbody className="text-sm divide-y divide-white/5">
              ...
              <tr><td className="p-4 text-gray-400">Loại chậu</td><td className="p-4">C6 / Nhựa dẻo</td><td className="p-4">C10 / C12</td><td className="p-4">C15 / Bầu đất</td></tr>
      ...
      {/* Tồn kho dự án lớn */}
      <div className="max-w-6xl mx-auto px-4 sm:px-8 mb-12">
        <h3 className="text-xs text-indigo-400 font-black uppercase tracking-widest mb-6">Tồn Kho Quy Mô Lớn ({projectBatches.length} lô &ge; 100 cây)</h3>
        <DuanClient projectBatches={projectBatches} />
      </div>

      {/* CTA */}
      <div className="fixed bottom-6 right-6 z-[60]">
        <a href="https://zalo.me/0849866686" target="_blank" rel="noreferrer"
           className="bg-indigo-600 hover:bg-indigo-500 text-white shadow-xl shadow-indigo-500/40 rounded-full px-6 py-4 font-black flex items-center gap-3 transition-transform hover:-translate-y-2 border border-indigo-400/50">
          ...
          <span className="uppercase tracking-wider text-xs">Liên hệ báo giá dự án</span>
        </a>
      </div>
      {/* Footer */}
      <footer className="max-w-6xl mx-auto px-4 sm:px-8 pt-8 pb-4 border-t border-white/10 mt-12">
